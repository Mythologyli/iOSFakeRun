...
internal static class Image
{
    public static bool MountImage(iDeviceHandle iDevice, LockdownClientHandle lockdownClient, string iosVersion)
    {
        ...
        {
            if (mounterInstance.mobile_image_mounter_new(iDevice, lockdownServiceDescriptor, out var mounterClient) != MobileImageMounterError.Success) return false;

            using (mounterClient)
            {
                try
                {
                    if (mounterInstance.mobile_image_mounter_lookup_image(mounterClient, "Developer", out var plist) != MobileImageMounterError.Success) return false;
                    ...
                    using (plist)
                    {
                        ...
                        if (!plistXml.Contains("Complete") || plistXml.Contains("ImageMountFailed")) return false;
                    }
                }
                finally
                {
                    if (mounterClient != null && !mounterClient.IsClosed) mounterInstance.mobile_image_mounter_hangup(mounterClient);
                }
            }
        }
